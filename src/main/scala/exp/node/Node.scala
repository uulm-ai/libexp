...
case class App[+S <: Stage,+T](stage: S, inputs: IndexedSeq[Node[S,_]], f: IndexedSeq[_] => T, effort: Effort = Effort.low) extends Node[S,T]{
...
case class Lift[+S <: Stage,+T](stage: S, p: Node[S,Stream[T]]) extends Node[S,T] {
...
case class Inject[+S <: Stage,+T] protected[node] (stage: S, p: S#Payload[T]) extends Node[S,T] {
